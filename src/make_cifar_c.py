...
# Code modified from:
# https://github.com/hendrycks/robustness/tree/master
...
def plasma_fractal(mapsize=32, wibbledecay=3):
    ...
    def fillsquares():
        ...
        maparray[stepsize // 2 : mapsize : stepsize, stepsize // 2 : mapsize : stepsize] = wibbledmean(
            squareaccum
        )
...
def elastic_transform(image, severity=1):
    ...
    dx = (
        gaussian(np.random.uniform(-1, 1, size=shape[:2]), c[1], mode="reflect", truncate=3) * c[0]
    ).astype(np.float32)
    dy = (
        gaussian(np.random.uniform(-1, 1, size=shape[:2]), c[1], mode="reflect", truncate=3) * c[0]
    ).astype(np.float32)
...
def get_corruptions(images_dir):
    test_data, test_labels = torch.load(images_dir, weights_only=False)
    test_data = np.transpose(test_data.numpy(), (0, 2, 3, 1))
    test_data = np.uint8(test_data * 255.0)
    corruptions_dic = {severity: [] for severity in range(1, 6)}
    labels_dic = {severity: [] for severity in range(1, 6)}

    selected_corruptions = [choice(list(d.keys())) for _ in test_data]
    for img_idx, (img, label) in enumerate(zip(test_data, test_labels)):
        method_name = selected_corruptions[img_idx]
        for severity in range(1, 6):
            corruption = lambda clean_img: d[method_name](clean_img, severity)
            pil_image = Image.fromarray(img)
            corrupted_img = corruption(pil_image)
            corruptions_dic[severity].append(np.uint8(corrupted_img))
            labels_dic[severity].append(label)

    for severity in range(1, 6):
        corruptions_dic[severity] = np.array(corruptions_dic[severity])
        labels_dic[severity] = np.array(labels_dic[severity])

    return corruptions_dic, labels_dic
