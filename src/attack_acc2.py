...
import torchattacks
from attacks2 import fgsm_attack ,pgd_attack, cw_attack, bim_attack, square_attack, deepfool_attack, evaluate_attack
import os
...
train_loader, val_loader, test_loader_tiny, test_loader_r= load_data(train_tiny_path, val_tiny_path,test_tiny_path,r_path)
...
def evaluate_and_save_all_attacks(models_to_train, test_loader_tiny, attacks_to_test, save_dir='/Users/joanacorreia/Desktop/AECD/Robustness-Metrics/models/Attacked_tensors/'):
    """
    ...
    for model_config in models_to_train:
        ...
        for config in attacks_to_test:
            ...
            adv_images_path = f"{save_dir}{model_name}_{attack_name}_{'_'.join(f'{k}_{v}' for k, v in params.items())}.pt"
            if os.path.exists(adv_images_path):
                ...
                print(f"Adversarial examples for {attack_name} attack on {model_name} not found. Skipping accuracy calculation.")
                continue
evaluate_and_save_all_attacks(
    models_to_train=models_to_train,
    test_loader_tiny=test_loader_tiny,
    attacks_to_test=attacks_to_test
)


def evaluate_attacks_saved(models_to_train, test_loader_tiny, attacks_to_test, save_dir='/Users/joanacorreia/Desktop/AECD/Robustness-Metrics/models/Attacked_tensors/'):
    """
    Evaluate all attacks on all models, save adversarial examples, and calculate adversarial accuracy.
    """
