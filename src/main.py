...
def main():
    ...
    for config in models_to_train[-3:]:
        ...
        logits_test_101, labels_test_101 = get_logits_and_labels(
            test_loader_101, model, model_name, "cifar10.1", device
        )
        logits_test_10c1, labels_test_10c1 = get_logits_and_labels(
            test_loader_10c1, model, model_name, "cifar10c1", device
        )
        logits_test_10c2, labels_test_10c2 = get_logits_and_labels(
            test_loader_10c2, model, model_name, "cifar10c2", device
        )
        logits_test_10c3, labels_test_10c3 = get_logits_and_labels(
            test_loader_10c3, model, model_name, "cifar10c3", device
        )
        logits_test_10c4, labels_test_10c4 = get_logits_and_labels(
            test_loader_10c4, model, model_name, "cifar10c4", device
        )
        logits_test_10c5, labels_test_10c5 = get_logits_and_labels(
            test_loader_10c5, model, model_name, "cifar10c5", device
        )
        ...
        datasets = {
            "train": (logits_train, labels_train),
        ...
        complex_diff = {}
        complex_diff["complexity_diff_train_val"] = calculate_metric_differences(
            complex_metrics["complexity_train"], complex_metrics["complexity_val"]
        )
        ...
        if not os.path.exists(f"{path_to_measures}/sharpness.pt"):
            train_accuracy = complex_metrics["complexity_train"]["accuracy"]
