...
def gradient_ascent_one_step(
    ...
    device: torch.device,
    num_batches: int = 3,  # Number of batches to process per ascent step
) -> float:
    """
...
def sharpness_sigma_search(
    ...
    for sd in range(search_depth):
        ...
        for _ in range(monte_carlo_iter):
            with apply_perturbation(model, sigma, noise_type=noise_type, magnitude_eps=None):
                ...
                for _ in range(ascent_steps):
                    ...
                    if noise_type == "uniform_magnitude_aware":
                        clip_perturbed_weights(model, sigma, list(original_state.values()), use_plus_one)
                    else:  # uniform standard
                        normalize_perturbed_weights(model, sigma, original_state)
