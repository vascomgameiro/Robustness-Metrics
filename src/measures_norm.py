...
def calculate_margin(model: nn.Module, device: str, dataloader: DataLoader):
    ...
    return torch.cat(margins).kthvalue(dataset_size // 10)[0]
...
def calculate_generalization_bounds(
    ...
    with torch.no_grad():
        ...
        measures["L_{1,inf} norm"] = (
            calculate_measure(**norm_settings, kwargs={"p": 1, "q": np.inf}) / margin**2
        )
        measures["Frobenius norm"] = calculate_measure(**norm_settings, kwargs={"p": 2, "q": 2}) / margin**2
        measures["L_{3,1.5} norm"] = (
            calculate_measure(**norm_settings, kwargs={"p": 3, "q": 1.5}) / margin**2
        )
        ...
        measures["Spectral norm"] = (
            calculate_measure(**opperator_settings, kwargs={"p": float("Inf")}) / margin**2
        )
        measures["L_1.5 operator norm"] = (
            calculate_measure(**opperator_settings, kwargs={"p": 1.5}) / margin**2
        )
        measures["Trace norm"] = calculate_measure(**opperator_settings, kwargs={"p": 1}) / margin**2

        # Path Norms
        input_size = (nchannels, img_dim, img_dim)
        measures["L1_path norm"] = (
            calculate_path_norm(model, device, p=1, input_size=input_size) / margin**2
        )
        measures["L1.5_path norm"] = (
            calculate_path_norm(model, device, p=1.5, input_size=input_size) / margin**2
        )
        measures["L2_path norm"] = (
            calculate_path_norm(model, device, p=2, input_size=input_size) / margin**2
        )
        ...
        measures["Frobenius Distance"] = (
            calculate_measure(**distance_settings, kwargs={"p": 2, "q": 2}) / margin**2
        )
        measures["Spectral Distance"] = (
            calculate_measure(**distance_settings, kwargs={"p": float("Inf")}) / margin**2
        )
